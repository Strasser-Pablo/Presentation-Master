...
\subsection{Reformulation des équations de Navier-Stokes}
...
\begin{block}{Chose nécessaire}
 \begin{itemize}
  ...
  \begin{equation*}
   \dot{x}=v
  \end{equation*}
  \item Imposer les conditions aux bords variable:
   \begin{align*}
 p_{surf}&=Av_{int}\\
 v_{surf}&=Bv_{int}
\end{align*}
...
Ses points fantômes sont obtenus par des méthodes d'extrapolations.
\begin{align*}
 p_{ext}&=Av_{int}\\
 v_{ext}&=Bv_{int}
\end{align*}
...
\shorthandoff{:;}
 \begin{tikzpicture}[node distance=5mm,line width=0.5mm]
  ...
  \node[below= of extrap,draw](gravity) {Gravité};
   ...
   \node[below= of conv,draw](viscosity) {Viscosité};
   \node[below= of viscosity,draw](projection) {Projection};
   \node[left= of gravity,draw,red](part){Mouvements des particules};
   \node[draw,fit=(conv) (gravity) (viscosity) (projection),label={180:{\rotatebox{90}{Accélération}}}](accel) {};
   \node[draw,fit=(accel) (part) (runginit),label={below:Runge-Kutta}] {};
   \draw[->] (init)--(runginit);
   \draw[->] (runginit)--(extrap);
...
\begin{enumerate}
 ...
 \item On extrapole le plus possible avec les cas vu précédemment.
...
\begin{enumerate}
 \item On extrapole les vitesses inconnues de la surface par moyenne des vitesses intérieur voisine.
...
\begin{frame}{Resumé de la méthode}
...
\begin{frame}{Resumé de la méthode (suite)}
...
\begin{frame}{Equations de Navier-Stokes}
...
\begin{block}{Exactement incompressible}
 \begin{enumerate}
  \item Projeter la vitesse pour être à divergence nulle.
  \item Obtenir une densité égal à la densitée voulue $\rho_0$.
 \end{enumerate}
\begin{equation*}
 \frac{\dot{\rho}}{\rho}=\nabla \cdot v
\end{equation*}
...
\begin{block}{Créer particule initiale}
	\begin{enumerate}
		...
		\item Choisir la masse des particules de sorte que la densitée soit bonne.
		\begin{enumerate}
			\item La densitée est donnée par:
			\begin{equation*}
